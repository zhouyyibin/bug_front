  ...
  <div>
    <a-card
      ...
      <div slot="extra" v-action="`project_add`">
        <a-button type="primary" @click="$router.push('/project/add')" ><a-icon type="plus" />添加项目</a-button>
      </div>
      ...
      >
        <div slot="senderBeans" slot-scope="text, record">
          <a-tag v-for="item in record.senderBeans" :key="item.id">{{item.name}}</a-tag>
        </div>
  ...
  methods: {
    handleDelete(id) {
      this.$confirm({
        ...
        cancelText: '取消',
        onOk: () => {
          api.project.remove(id).then(() => this.$refs.table.refresh(1))
        },
        onCancel: () => {
          console.log('Cancel')
        }
